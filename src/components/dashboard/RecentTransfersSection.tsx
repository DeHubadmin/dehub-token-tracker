...
const RecentTransfersSection: React.FC<RecentTransfersSectionProps> = ({
  ...
  const filteredTransfers = searchQuery ? recentTransfers.filter(tx => tx.from.toLowerCase().includes(searchQuery.toLowerCase()) || tx.to.toLowerCase().includes(searchQuery.toLowerCase()) || tx.txHash.toLowerCase().includes(searchQuery.toLowerCase())) : recentTransfers;
  ...
  return <>
      ...
      <Card className="bg-dehub-card border-dehub-card-border mb-8">
        <CardHeader>
          ...
          <div className="mt-4">
            <Input placeholder="Search by address or tx hash..." className="bg-dehub-input border-dehub-border text-white" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
          </div>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {displayedTransfers.map((tx, index) => <TableRow key={`${tx.txHash}-${tx.chain}-${index}`} className="border-b border-white/10">
